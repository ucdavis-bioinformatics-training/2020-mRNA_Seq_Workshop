...
```{r, include = F}
# It turns out that the "raw" versions are untrimmed reads that have been mapped and summarized through the same set of steps.
# The scripts for doing this aren't written yet.
...
# Read salmon.counts and strip versions from the ENSEMBL id:
...
snames
factor1 <- paste("mouse", sapply(strsplit(snames, '_'), '[', 2), sep='_')
factor2 <- sapply(strsplit(snames, "_"), '[', 3)
factor3 <- sapply(strsplit(snames, "_"), "[", 4)

group <- interaction(factor1, factor2, factor3)

#Setup colnames so that factors are delimited by "."
...
cpms.star.l = pivot_longer(data.frame(ENSEMBLID=rownames(cpms.star), cpms.star), 
                           names_to=c("Sample","Genotype","Monocytes"), names_sep='\\.', 
...
df = inner_join(cpms.star.l, cpms.salmon.l, by=c("ENSEMBLID", "Sample", "Genotype", "Monocytes"))
...
ggplot(df.subset, aes(x=STAR, y=Salmon)) +
  ...
  facet_grid(rows= vars(Sample), cols=vars(Monocytes))
```
...
```{r, stopgap}
DE.star = data.frame(adj.P.Val=c(1,1))
DE.salmon = data.frame(adj.P.Val = c(1,1))
```
